...
Scene: flat world, night frozen, mob spawning off, 500 NoAI zombies in a grid plus 4 NoAI
Monster Hunter villagers parked out of the zombies' reach. After a warm-up the script samples the
server's own mean tick time (last 100 ticks) every 5 s.

Usage: python bench.py <forge1201|neo1211> <mod jar or 'none'> <label>
"""
...
for x in range(-24, 26, 2):
    for z in range(-20, 20, 2):
        ...
        count += 1
for x, z in [(38, 38), (38, -38), (-38, 38), (-38, -38)]:
    send(f'summon minecraft:villager {x} -60 {z} {{NoAI:1b,PersistenceRequired:1b,'
